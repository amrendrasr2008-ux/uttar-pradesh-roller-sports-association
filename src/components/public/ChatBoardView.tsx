...
export const ChatBoardView: React.FC<ChatBoardViewProps> = ({ onNavigate }) => {
  const { language } = useLanguage();
  const { role, activeSkater, isAdminAuthenticated } = useAuth();

  const [activeTab, setActiveTab] = useState<'ai' | 'community' | 'contact'>('ai');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('uprsa_chat_history_v1', JSON.stringify(chatHistory));
    } catch (e) {}
  }, [chatHistory]);
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const replyText = data.reply || 'माफ़ करें, इस समय प्रतिक्रिया प्राप्त नहीं हो सकी। कृपया पुनः प्रयास करें।';
  ...
  const getRoleBadge = (roleStr: string) => {
    switch (roleStr) {
      case 'official':
        return <span className="px-2 py-0.5 text-[10px] font-black bg-purple-900/80 text-purple-200 border border-purple-500/50 rounded-full flex items-center gap-1"><ShieldCheck className="w-3 h-3 text-purple-400" /> Official</span>;
      case 'coach':
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      {/* Top Banner Header */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-amber-950/60 border border-amber-500/40 rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="flex items-center gap-1.5 text-xs text-emerald-400 font-bold bg-emerald-950/60 border border-emerald-500/40 px-2.5 py-0.5 rounded-full">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                Live Online
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-white tracking-tight">
              {language === 'hi' ? 'UPRSA लाइव चैट व कम्युनिटी बोर्ड' : 'UPRSA Live Chat & Community Board'}
            </h1>
            <p className="text-sm sm:text-base text-slate-300 font-medium mt-1 max-w-3xl">
              {language === 'hi'
                ? 'स्केटिंग पंजीकरण, आयु वर्ग नियम, टूर्नामेंट व तकनीकी नियमों पर सवाल पूछें अथवा राज्य भर के स्केटर्स व कोचों से विचार साझा करें।'
                : 'Ask questions about skater registration, age categories, rules, and tournaments, or interact with skaters, coaches, and parents across Uttar Pradesh.'}
            </p>
          </div>

          <div className="flex items-center gap-2 flex-wrap">
            <a
              href="https://wa.me/919415011223"
              target="_blank"
              rel="noopener noreferrer"
              className="px-4 py-2.5 bg-[#25D366] hover:brightness-110 text-slate-950 font-black text-xs sm:text-sm rounded-xl flex items-center gap-2 shadow-lg transition"
            >
              <MessageCircle className="w-4 h-4" /> व्हाट्सएप हेल्पलाइन
            </a>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col h-[720px]">
        
        {/* Navigation Tabs */}
        <div className="bg-slate-950 border-b border-slate-800 px-4 py-3 flex items-center justify-between gap-3 overflow-x-auto scrollbar-none">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{language === 'hi' ? '🤖 AI स्केटिंग हेल्पलाइन' : '🤖 AI Assistant'}</span>
            </button>
            ...
            >
              ...
              <span className="px-1.5 py-0.2 bg-emerald-500/20 text-emerald-400 rounded-full text-xs font-black">
          ...
          {activeTab === 'ai' && (
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-red-400 text-xs font-bold transition flex items-center gap-1.5 border border-slate-800"
            >
              <RotateCcw className="w-3.5 h-3.5" /> रीसेट चैट
            </button>
          )}
          ...
          {activeTab === 'community' && (
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs sm:text-sm rounded-xl shadow-lg flex items-center gap-1.5 transition shrink-0"
            >
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-slate-950/50">
            ...
            <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4">
              ...
              <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-2xl">
                <p className="text-xs font-bold text-amber-400 mb-2 flex items-center gap-2">
                  ...
                  {language === 'hi' ? 'अक्सर पूछे जाने वाले सवाल (तुरंत उत्तर के लिए क्लिक करें):' : 'Frequently Asked Topics:'}
                </p>
                <div className="flex flex-wrap gap-2">
                  {quickQuestions.map((q, idx) => (
                    <button
                      ...
                      disabled={isTyping}
                      className="px-3 py-2 bg-slate-950 hover:bg-amber-500/20 text-slate-300 hover:text-amber-300 text-xs font-semibold rounded-xl border border-slate-800 hover:border-amber-500/40 transition text-left"
                    >
                      {q.label}
                    </button>
                  ))}
              ...
              {chatHistory.map(msg => (
                ...
                >
                  ...
                  <div className={`max-w-[80%] rounded-3xl px-5 py-3.5 text-sm leading-relaxed shadow-lg ${
                    msg.role === 'user'
              ...
              {isTyping && (
                <div className="flex items-center gap-3 text-slate-400 text-xs pl-2">
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl px-4 py-3 flex items-center gap-2 shadow-md">
                    <span className="w-2 h-2 rounded-full bg-amber-400 animate-bounce" style={{ animationDelay: '0ms' }} />
                    <span className="w-2 h-2 rounded-full bg-amber-400 animate-bounce" style={{ animationDelay: '150ms' }} />
                    <span className="w-2 h-2 rounded-full bg-amber-400 animate-bounce" style={{ animationDelay: '300ms' }} />
                    <span className="text-xs text-amber-300 ml-1">UPRSA AI उत्तर लिख रहा है...</span>
                  </div>
                </div>
              )}
            ...
            >
              <input
                ...
                disabled={isTyping}
                className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl px-5 py-3.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition"
              />
              <button
                type="submit"
                disabled={!inputMessage.trim() || isTyping}
                className="px-6 py-3.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 disabled:opacity-50 text-slate-950 font-black rounded-2xl transition flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20"
              >
        ...
        {/* Tab 2: Community Chat Stream */}
        {activeTab === 'community' && (
          <div className="flex-1 flex flex-col min-h-0 bg-slate-950/50 p-4 sm:p-6 overflow-y-auto space-y-4">
            
            {showNewPostForm && (
              <form 
                onSubmit={handleCreatePost}
                className="bg-slate-900 border border-emerald-500/40 rounded-3xl p-5 space-y-4 shadow-2xl animate-fadeIn"
              >
                <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                  <h4 className="text-base font-black text-emerald-400 flex items-center gap-2">
                    <Plus className="w-5 h-5" /> नया कम्युनिटी संदेश पोस्ट करें
                  </h4>
                  <button 
                    type="button" 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">आपका नाम (Your Name)</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">भूमिका (Role)</label>
                    <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">संदेश (Message)</label>
                  <textarea
                    rows={3}
                    value={postMessage}
                ...
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-400 font-bold">कैटेगरी:</span>
                    <select 
                      value={postCategory}
                      onChange={e => setPostCategory(e.target.value as any)}
                      className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-amber-400 font-bold"
                    >
                  ...
                  >
                    <Send className="w-4 h-4" /> पोस्ट करें
                  </button>
                </div>
              </form>
            )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900 border border-slate-800 p-3 rounded-2xl">
              ...
              <div className="flex items-center gap-1.5 text-xs">
                {['all', 'tournament', 'training', 'inquiry'].map(cat => (
                  <button
                    key={cat}
                    onClick={() => setFilterCategory(cat)}
                    className={`px-3 py-1.5 rounded-xl font-bold transition ${filterCategory === cat ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:bg-slate-800'}`}
                  >
                    {cat === 'all' ? 'सभी' : cat}
                  </button>
                ))}
            ...
            <div className="space-y-3.5">
              {filteredPosts.map(post => (
                <div 
                  key={post.id}
                  className="bg-slate-900 border border-slate-800 hover:border-amber-500/30 rounded-2xl p-4 space-y-3 transition shadow-xl"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-slate-800 border border-slate-700 flex items-center justify-center font-black text-amber-400 text-sm">
                  ...
                  <p className="text-sm text-slate-200 leading-relaxed whitespace-pre-wrap pl-12">
                    {post.message}
                  </p>

                  <div className="flex items-center justify-between pt-2 border-t border-slate-800/80 pl-12">
                    <button
                      onClick={() => handleLikePost(post.id)}
                      className={`flex items-center gap-2 text-xs px-3 py-1.5 rounded-xl transition ${
                        post.userLiked 
        ...
        {/* Tab 3: Official Helpdesk */}
        {activeTab === 'contact' && (
          <div className="flex-1 bg-slate-950/50 p-6 overflow-y-auto space-y-6">
            
            {/* Admin Quick Action if authenticated */}
            {isAdminAuthenticated && (
              <div className="flex items-center justify-between bg-amber-500/15 border border-amber-500/30 px-4 py-2.5 rounded-2xl text-xs sm:text-sm">
                <div className="flex items-center gap-2 text-amber-300 font-bold">
                  <ShieldCheck className="w-5 h-5 text-amber-400" />
                  <span>एडमिन: आप यह हेल्पडेस्क विवरण एडमिन पैनल से सीधे संपादित व अपडेट कर सकते हैं।</span>
                </div>
                {onNavigate && (
                  <button
                    onClick={() => onNavigate('admin')}
                    className="px-3.5 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black rounded-xl text-xs flex items-center gap-1.5 shadow"
                  >
                    <Edit3 className="w-4 h-4" /> विवरण संपादित करें (Admin Edit)
                  </button>
                )}
              </div>
            )}
            ...
            {/* Secretariat Header */}
            <div className="bg-gradient-to-br from-amber-500/10 via-slate-900 to-slate-900 border border-amber-500/30 rounded-3xl p-6">
              <h4 className="text-base sm:text-lg font-black text-amber-400 flex items-center gap-2.5 mb-2">
                <Trophy className="w-5 h-5 text-amber-400" /> {settings.helpdeskSecretariatTitle || 'उत्तर प्रदेश रोलर स्पोर्ट्स एसोसिएशन (UPRSA) सचिवालय'}
              </h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-900 border border-emerald-500/40 rounded-3xl p-6 space-y-3">
                <div className="flex items-center gap-3 text-emerald-400 font-bold text-base">
                  <MessageCircle className="w-6 h-6" /> {settings.helpdeskWhatsappTitle || 'आधिकारिक व्हाट्सएप हेल्पडेस्क'}
                </div>
                <p className="text-sm text-slate-300 leading-relaxed">
                  {settings.helpdeskWhatsappDesc || 'पंजीकरण, आईडी कार्ड या परिणाम संबंधित तुरंत सहायता के लिए हमारे आधिकारिक व्हाट्सएप नंबर पर सीधे संपर्क करें।'}
                </p>
                <a
                  href={`https://wa.me/${(settings.whatsappSupportNumber || '+919415011223').replace(/[^0-9]/g, '')}?text=Namaste%20UPRSA%20Helpline,%20I%20have%20a%20query.`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-3 bg-[#25D366] text-slate-950 font-black text-sm rounded-2xl hover:brightness-110 transition shadow-lg"
                >
                  <ExternalLink className="w-4 h-4" /> व्हाट्सएप चैट शुरू करें ({settings.whatsappSupportNumber || '+91 94150 11223'})
                </a>
              </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-3">
                <div className="flex items-center gap-3 text-amber-400 font-bold text-base">
                  <MapPin className="w-6 h-6" /> {settings.helpdeskLucknowTitle || 'राज्य मुख्यालय (Lucknow)'}
                </div>
                <p className="text-sm text-slate-300 leading-relaxed">
                  {settings.helpdeskLucknowAddress || 'के.डी. सिंह बाबू स्टेडियम स्केटिंग कॉम्प्लेक्स, हज़रतगंज, लखनऊ, उत्तर प्रदेश - 226001'}
                </p>
                <div className="flex items-center gap-2 text-sm font-bold text-slate-200 flex-wrap">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-3">
                <div className="flex items-center gap-3 text-blue-400 font-bold text-base">
                  <MapPin className="w-6 h-6" /> {settings.helpdeskWesternTitle || 'वेस्टर्न यूपी केंद्र (Noida)'}
                </div>
                <p className="text-sm text-slate-300 leading-relaxed">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-3">
                <div className="flex items-center gap-3 text-purple-400 font-bold text-base">
                  <Mail className="w-6 h-6" /> {settings.helpdeskEmailTitle || 'आधिकारिक ईमेल सहायता'}
                </div>
                <p className="text-sm text-slate-300">
                  {settings.helpdeskEmailDesc || 'आधिकारिक पत्राचार, सत्यापन एवं क्लब संबद्धता हेतु ईमेल भेजें।'}
                </p>
                <div className="flex items-center gap-2 text-sm font-bold text-slate-200">
            ...
            {onNavigate && (
              <div className="pt-4 border-t border-slate-800 flex flex-wrap gap-3">
                <button
                  ...
                  className="px-4 py-2.5 bg-amber-500 text-slate-950 font-black text-xs sm:text-sm rounded-xl flex items-center gap-2 shadow"
                >
                  <ChevronRight className="w-4 h-4" /> स्केटर रजिस्ट्रेशन
                </button>
                <button
                  onClick={() => onNavigate('tournaments')}
                  className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs sm:text-sm rounded-xl flex items-center gap-2"
                >
                  <Calendar className="w-4 h-4 text-amber-400" /> प्रतियोगिताएं देखें
                </button>
                <button
                  onClick={() => onNavigate('results')}
                  className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs sm:text-sm rounded-xl flex items-center gap-2"
                >
                  <Award className="w-4 h-4 text-emerald-400" /> लाइव रिजल्ट व स्कोर
                </button>
              </div>
            )}
